...
export const getFilteredLetters = async ({
  ...
  const rawLetters = await prisma.letter.findMany({
    where: {
      parentLetterId: null,
      receiverId: isMine ? userId : undefined,
    ...
    include: {
      Favorite: true,
      User_Letter_senderIdToUser: isMine,
      User_Letter_receiverIdToUser: isFriend,
    },
  });
